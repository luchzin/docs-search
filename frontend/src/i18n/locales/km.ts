const documents = {
  dropzoneTitle: "ទម្លាក់ឯកសារទីនេះ ឬចុចដើម្បីបង្ហោះ",
  dropzoneSub: "គាំទ្រឯកសារ PDF, DOCX, TXT, MD, CSV, JSON រហូតដល់ 20MB",
  pdfOnly: "PDF, DOCX, TXT, MD, CSV, JSON",
  supportedFormats: "PDF, DOCX, TXT, MD, CSV, JSON",
  onlyPdfSupported: "គាំទ្រតែឯកសារ PDF, DOCX, TXT, MD, CSV, JSON និង LOG ប៉ុណ្ណោះ",
  uploading: "កំពុងបង្ហោះ និងដំណើរការទិន្នន័យ...",
  ready: "រួចរាល់",
  remove: "លុបចេញ"
};

export default {
  common: {
    appName: "DocsSearch AI",
    settings: "ការកំណត់",
    cancel: "បោះបង់",
    save: "រក្សាទុក",
    delete: "លុប",
    loading: "កំពុងផ្ទុក...",
    error: "កំហុស",
    success: "ជោគជ័យ"
  },
  header: {
    title: "ជំនួយការ AI ឯកសារ",
    settingsTitle: "ការកំណត់ & ចំណូលចិត្ត",
    toggleSidebar: "បិទ/បើករបារចំហៀង",
    downloadApp: "ទាញយកកម្មវិធី",
    toggleDarkMode: "ប្តូររបៀបងងឹត",
    signInRegister: "ចូលប្រើ / ចុះឈ្មោះ"
  },
  sidebar: {
    title: "DocsSearch AI",
    chats: "ការជជែក",
    docs: "ឯកសារ",
    newChat: "ការជជែកថ្មី",
    today: "ថ្ងៃនេះ",
    yesterday: "ម្សិលមិញ",
    previous7Days: "៧ ថ្ងៃមុន",
    older: "ចាស់ជាងនេះ",
    noChats: "មិនទាន់មានប្រវត្តិជជែកនៅឡើយទេ",
    deleteChat: "លុបការជជែក",
    renameChat: "ប្តូរឈ្មោះការជជែក"
  },
  chat: {
    inputPlaceholder: "សួរពាក្យសុំ ឬសំណួរអំពីឯកសាររបស់អ្នក...",
    sendMessage: "ផ្ញើសារ",
    send: "ផ្ញើ",
    thinking: "កំពុងគិត...",
    clearMessages: "សម្អាតសារជជែក",
    uploadPrompt: "បង្ហោះឯកសារដើម្បីចាប់ផ្តើមសួរសំណួរ",
    noDocumentsFound: "មិនមានទិន្នន័យឯកសារ។ សូមបង្ហោះឯកសារជាមុនសិន។",
    sourcesUsed: "ប្រភពដែលបានប្រើ",
    userRole: "អ្នក",
    assistantRole: "DocsSearch AI",
    welcomeTitle: "សូមស្វាគមន៍មកកាន់ DocsSearch AI",
    welcomeSub: "បង្ហោះឯកសារ (PDF, DOCX, TXT, MD, CSV, JSON) និងសួរសំណួរដើម្បីទទួលបានចម្លើយ AI យ៉ាងច្បាស់លាស់ជាមួយប្រភពយោង។",
    startUploadNotice: "សូមបង្ហោះឯកសារយ៉ាងហោចណាស់មួយក្នុងរបារចំហៀងដើម្បីចាប់ផ្តើមសួរសំណួរ។",
    jumpToLatest: "លោតទៅសារចុងក្រោយ",
    answersNotice: "AI អាចបង្កើតកំហុសបាន។ សូមផ្ទៀងផ្ទាត់ព័ត៌មាន និងប្រភពយោងសំខាន់ៗ។"
  },
  documents,
  docs: documents,
  settings: {
    title: "ការកំណត់កម្មវិធី",
    subtitle: "កំណត់ម៉ូឌែល AI កូនសោ API រូបរាង ភាសា និងទិន្នន័យកន្លែងធ្វើការរបស់អ្នក។",
    backToChat: "ត្រឡប់ទៅការជជែក",
    modelTab: "ម៉ូឌែល AI & កូនសោ",
    appearanceTab: "រូបរាង",
    languageTab: "ភាសា",
    accountTab: "គណនី & ការចូលប្រើ",
    dataTab: "ទិន្នន័យ & ការផ្ទុក",
    userAccount: "គណនីអ្នកប្រើប្រាស់",
    manageAccountSub: "គ្រប់គ្រងវគ្គចូលប្រើ និងព័ត៌មានលម្អិតនៃគណនីរបស់អ្នក។",
    signInSyncNotice: "ចូលប្រើដើម្បីធ្វើបច្ចុប្បន្នភាពការជជែកឯកសារ និងការកំណត់របស់អ្នក។",
    loggedInAs: "បានចូលប្រើប្រាស់ជា",
    notLoggedIn: "អ្នកមិនទាន់បានចូលប្រើប្រាស់នៅឡើយទេ។",
    logoutBtn: "ចាកចេញ",
    selectModel: "ជ្រើសរើសម៉ូឌែល AI",
    active: "សកម្ម",
    defaultTag: "លំនាំដើម",
    modelSub: "ជ្រើសរើសម៉ូឌែលសកម្មមួយដើម្បីដោះស្រាយការជជែក និងការស្វែងរកឯកសារ។ Google Gemini ត្រូវបានកំណត់ជាលំនាំដើម។",
    defaultGeminiNotice: "Google Gemini គឺជា AI លំនាំដើម (មិនចាំបាច់បំពេញទម្រង់កូនសោ API ទេ)",
    defaultGeminiSub: "Google Gemini ត្រូវបានកំណត់ជាម៉ូឌែល AI លំនាំដើមនៅក្នុងកម្មវិធីនេះ។ ការតភ្ជាប់ API របស់ម៉ាស៊ីនបម្រើត្រូវបានកំណត់ស្វ័យប្រវត្តិ ដូច្នេះអ្នកប្រើប្រាស់មិនចាំបាច់បញ្ចូលកូនសោ API ឡើយ។",
    systemIntegrationNotice: "ការតភ្ជាប់ប្រព័ន្ធសកម្ម — មិនចាំបាច់បញ្ចូលកូនសោ API សម្រាប់ Google Gemini ទេ។",
    apiKeyFor: "កូនសោ API សម្រាប់ {provider}",
    apiKeySub: "កំណត់កូនសោ API សម្រាប់ម៉ូឌែលដែលបានជ្រើសរើស ({model})។",
    apiKeyHeader: "កូនសោ API {provider}",
    requiredForModel: "ត្រូវការសម្រាប់ {model}",
    apiKeySavedLocally: "រក្សាទុកក្នុងកម្មវិធីរុករកក្នុងស្រុក",
    activeKeySaved: "បានរក្សាទុកកូនសោសកម្ម",
    keySet: "បានកំណត់កូនសោ ✓",
    optionalRequired: "ជម្រើស / ត្រូវការ",
    enterKeyPlaceholder: "បញ្ចូលកូនសោ API សម្រាប់ {provider}...",
    submitNotice: "ចុចខាងក្រោមដើម្បីបញ្ជូន និងរក្សាទុកកូនសោ API សម្រាប់ {provider}។",
    saveApiKeyBtn: "រក្សាទុក & បញ្ជូនកូនសោ API",
    keySavedSuccess: "បានរក្សាទុកកូនសោ API សម្រាប់ {provider} ដោយជោគជ័យ!",
    colorTheme: "ពណ៌រូបរាង",
    colorThemeSub: "ផ្លាស់ប្តូររវាងរបៀបភ្លឺ និងរបៀបងងឹត។",
    lightMode: "របៀបភ្លឺ",
    darkMode: "របៀបងងឹត",
    selectLanguage: "ភាសាចំណុចប្រទាក់",
    selectLanguageSub: "ជ្រើសរើសភាសាកម្មវិធីដែលអ្នកចូលចិត្ត។",
    langEnglish: "English (អង់គ្លេស)",
    langKhmer: "ភាសាខ្មែរ (Khmer)",
    clearStorage: "សម្អាតការផ្ទុកកម្មវិធីក្នុងស្រុក",
    clearStorageSub: "អំពើប្រព្រឹត្តិ​នេះនឹងកំណត់ប្រវត្តិជជែក ការកំណត់ម៉ូឌែល និងកូនសោ API ឡើងវិញ។",
    clearStorageBtn: "សម្អាតការផ្ទុក",
    confirmClearStorage: "តើអ្នកប្រាកដជាចង់សម្អាតប្រវត្តិជជែក និងឯកសារក្នុងស្រុកទាំងអស់ឬ?"
  },
  auth: {
    title: "ត្រូវការការផ្ទៀងផ្ទាត់",
    description: "សូមចូល ឬបង្កើតគណនីដើម្បីចូលប្រើការជជែក ឯកសារ និងការកំណត់។",
    login: "ចូល",
    register: "ចុះឈ្មោះ",
    email: "អាសយដ្ឋានអ៊ីមែល",
    password: "ពាក្យសម្ងាត់",
    name: "ឈ្មោះពេញ",
    fullName: "ឈ្មោះពេញ",
    signIn: "ចូលប្រើ",
    createAccount: "បង្កើតគណនី",
    submitLogin: "ចូលប្រើ",
    submitRegister: "បង្កើតគណនី",
    dontHaveAccount: "មិនទាន់មានគណនីមែនទេ? ចុះឈ្មោះ",
    alreadyHaveAccount: "មានគណនីរួចហើយមែនទេ? ចូលប្រើ"
  }
};
